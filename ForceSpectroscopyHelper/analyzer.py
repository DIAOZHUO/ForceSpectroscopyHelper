...
class GridAnalyzer(BaseAnalyzer):
    ...
    def CalcFMap(self, fileName, param:formula.measurement_param):
        cvt = PakConvertor(fileName, default_project_path, False)
        ...
        for i in range(0, xy[0]):
            for j in range(0, xy[1]):
                ...
                x = np.flipud(x)
                y = np.flipud(y)
                f = formula.CalcForceCurve(y, param)
...
class DulcineaAnalyzer(BaseAnalyzer):
    ...
    def __SaveFile(self, fileName, doFitting=True):
    ...
    def GetDfMap(self, fileName, N=1):
        ...
        # z_offset = float(get_value("ZScaleOffset"))
        # z_amp = float(get_value("ZAmplitude").replace("V", ""))
        # k = float(get_value("Forceconstant").replace("N/m", ""))
        # f0 = float(get_value("Samplingfrequency").replace("Hz", ""))
        ...
        return data, index
        # pd.DataFrame(data, index=index).to_csv(fileName + ".csv")

    def CalcFMap(self, fileName, saveFileName, N=1, f0=None, amp=None, k=None):
        result = self.__SaveFile(fileName, False)
        ...
        cvt = PakConvertor(saveFileName, default_project_path, False)
