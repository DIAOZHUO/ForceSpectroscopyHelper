...
class GridConvertor(BaseConvertor):
    ...
    def __init__(self, fileName="", data_path="./Resource/Data/"):
...
class StandardConvertor(BaseConvertor):
    ...
    def __init__(self, I=1024, Width=20, fileName="", DataCount=1024, data_path="./Resource/Data/"):
        BaseConvertor.__init__(self, fileName, data_path)
...
class PakConvertor(BaseConvertor):
    ...
    def __init__(self, fileName="", data_path="./Resource/Data/", autoLoad=True):
    ...
    def get_y_data(self, point=(0,0)):
        y = self.data_dict[point].y
        return np.array(y)

    def set_header(self, header:PakHeader):
    ...
    def save(self):
        if self.header_key not in self.data_dict:
            raise ValueError("Save file need a header, use set_header(type(PakHeader)) function")
    ...
    def reconstruct_map(self):
        ...
        if type(size) is not int and len(size) == 2:
            map = np.zeros(shape=(size[0], size[1], self.header.data_count))
            for i in range(size[0]):
                for j in range(size[1]):
                    map[j, i, :] = self.data_dict[(i, j)].y

        if type(size) is int:
            map = np.zeros(shape=(size, self.header.data_count))
            for i in range(1, size+1):
                    map[:, i-1] = self.data_dict[i].y
        else:
            raise NotImplementedError()
        return map
